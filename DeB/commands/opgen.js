...
const customCooldownHours = 2; // Change the value as desired
const genCooldownsec = customCooldownHours * 60 * 60; // Calculate the cooldown in seconds

module.exports = {
  name: 'opgen', // Command name
  description: 'Generate a specified service if stocked (opgen).', // Command description
  ...
  execute(message, args) {
    ...
    try {
      message.client.channels.cache.get(config.opgenChannel).id; // Try to get the channel's id
    } catch (error) {
    ...
    if (message.channel.id === config.opgenChannel) {
      // If the user has a cooldown on the command based on their rank
      let cooldown;
      switch (message.member.roles.cache.some(role => role.name === 'admghbnin')) {
        case true:
      ...
      if (generated.has(message.author.id) && generated.get(message.author.id) > Date.now()) {
        ...
        const filePath = `${__dirname}/../opstock/${args[0]}.txt`;
...
//maked by Cracky ! Big Thanks To Blockween 👍